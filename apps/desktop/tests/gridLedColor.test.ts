...
import { BLACK_COLOR, SPARKS_COLOR, WORLDS_COLOR, type RuntimeSnapshot } from "@octessera/device-contracts";
import { gridLedColor } from "../src/ui/gridLedColor";

function frame(rgb: readonly number[], settings: RuntimeSnapshot["settings"]): RuntimeSnapshot {
  return {
...
test("desktop grid LED color applies grid brightness", () => {
  assert.deepEqual(gridLedColor(frame(SPARKS_COLOR, settings({ gridBrightness: 50 })), 0), { r: 128, g: 106, b: 36 });
});
...
test("desktop grid LED color keeps non-black cells visible at zero brightness", () => {
  assert.deepEqual(gridLedColor(frame(WORLDS_COLOR, settings({ gridBrightness: 0 })), 0), { r: 4, g: 8, b: 2 });
});

test("desktop grid LED color keeps dimmed cells visibly dimmer", () => {
  assert.deepEqual(gridLedColor(frame(WORLDS_COLOR, settings({ gridBrightness: 0, ledsDimmed: true })), 0), { r: 1, g: 2, b: 1 });
});
