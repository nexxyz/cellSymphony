...
use playback_runtime::{HostAdapter, HostMessage, RuntimeAudioCommand, RuntimePlatformEffect, RuntimeStoreResult};
...
fn platform_effect_audio_command_reaches_audio_queue() {
    ...
    let follow_ups = adapter
        .handle_platform_effect(&RuntimePlatformEffect::AudioCommand {
            command: RuntimeAudioCommand::MomentaryFxStop { id: "preview".into() },
        })
        .unwrap();
    assert!(follow_ups.is_empty());
    assert!(matches!(rx.try_recv().unwrap(), QueuedAudioEvent::MomentaryFxStop { id } if id == "preview"));
}
...
fn deferred_default_save_flushes_runtime_result() {
    ...
    let follow_ups = adapter
        ...
        .unwrap();
    assert!(follow_ups.is_empty());
    adapter.pending_default_save = adapter.pending_default_save.take().map(|(payload, _)| (payload, Instant::now()));
    let follow_ups = adapter.flush_due_default_save().unwrap();
    assert_eq!(follow_ups, vec![HostMessage::RuntimeResult { result: RuntimeStoreResult::SaveDefaultResult { ok: true, is_auto: Some(true) } }]);
...
fn malformed_default_load_returns_store_error() {
    ...
    adapter.store_dir = temp_dir.clone();
    let follow_ups = adapter.handle_platform_effect(&RuntimePlatformEffect::StoreLoadDefault).unwrap();
    assert!(matches!(&follow_ups[..], [HostMessage::RuntimeResult { result: RuntimeStoreResult::StoreError { message } }] if message.starts_with("Default load failed:")));
    let _ = std::fs::remove_dir_all(temp_dir);
}
...
fn midi_panic_returns_native_status_result() {
    let (mut adapter, _) = test_adapter();
    let follow_ups = adapter.handle_platform_effect(&RuntimePlatformEffect::MidiPanic).unwrap();
    assert_eq!(follow_ups, vec![HostMessage::RuntimeResult { result: RuntimeStoreResult::MidiStatus { ok: true, message: Some("Panic sent".into()), selected_out_id: None, selected_in_id: None } }]);
}
