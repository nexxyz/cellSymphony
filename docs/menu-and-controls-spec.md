...
- `System > Updates > Check` emits `RuntimePlatformEffect::UpdateCheck` without confirmation. On Pi, the host runs `octessera-update check` through the platform worker and reports updater output as a toast/result. On desktop, it opens the GitHub releases page. `Apply` and `Rollback` confirm first, then emit `RuntimePlatformEffect::UpdateApply` or `RuntimePlatformEffect::Rollback`. On Pi, apply installs the latest validated device release and restarts Octessera; rollback switches to the previous recorded release and restarts Octessera. Desktop apply and rollback report unsupported.
