...
#[test]
pub(crate) fn auto_map_is_disabled_in_l2_sense_and_unbound_toast_uses_short_format() {
    let mut runner = NativeRunner::new(NativeRunnerConfig::default()).unwrap();
...
#[test]
pub(crate) fn custom_binding_is_used_on_l1_non_mapped_rows_even_when_auto_map_is_enabled() {
    let mut runner = NativeRunner::new(NativeRunnerConfig::default()).unwrap();
...
#[test]
pub(crate) fn auto_map_l1_life_turn_and_press_follow_behavior_context() {
    let mut runner = NativeRunner::new(NativeRunnerConfig::default()).unwrap();
