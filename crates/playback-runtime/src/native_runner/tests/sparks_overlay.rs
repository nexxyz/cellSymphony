...
pub(crate) fn fn_overlay_shows_active_layers_and_sparks_page_options() {
    ...
    assert_eq!(*active_layer, led_rgb(platform_core::palette::TONES));
    assert_eq!(
        *none_layer,
        led_rgb(dim_rgb(platform_core::palette::SYSTEM, 4))
    );
    assert_eq!(configured_layer, active_layer);
    assert_eq!(*selected_page, led_rgb(platform_core::palette::WORLDS));
...
pub(crate) fn fn_overlay_highlights_active_layer_when_not_in_sparks_mode() {
    ...
    assert_eq!(*configured_layer, led_rgb(platform_core::palette::WORLDS));
    assert_eq!(
        *sparks_page,
        led_rgb(dim_rgb(platform_core::palette::SPARKS, 4))
    );
    assert!(middle_cell["r"].as_i64().unwrap() < 70);
    assert!(middle_cell["g"].as_i64().unwrap() < 70);
    assert!(middle_cell["b"].as_i64().unwrap() < 70);
...
pub(crate) fn fn_overlay_dims_fx_grid_cells_when_sparks_mode_is_fx() {
    ...
    assert_eq!(*fx_page, led_rgb(platform_core::palette::WORLDS));
