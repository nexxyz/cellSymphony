...
fn native_menu_help_targets_resolve_to_specific_tsv_rows() {
    ...
    for config in representative_help_configs() {
        ...
        targets.extend(menu.help_targets().into_iter().filter(|target| target.kind != "action" || !target.key.is_empty()));
    }
    ...
    missing.extend(targets.into_iter().filter(|target| crate::native_help::resolve_native_help_entry(target).is_none()).map(|target| format!("{} {} {}", target.kind, target.key, target.path)));
    missing.sort();
    missing.dedup();
