...
fn voice_instrument_rows_expose_configuration_groups() {
    ...
    assert!(snapshot.lines.windows(2).any(|pair| pair[0] == "  Type:" && pair[1].trim() == "synth"));
    assert!(snapshot.lines.iter().any(|line| line == "> Synth"));
    assert!(!snapshot.lines.iter().any(|line| line == "> Sampler"));
    assert!(menu.current_siblings().iter().any(|item| item.label == "Clone"));
    assert!(menu.current_siblings().iter().any(|item| item.label == "Reset"));
...
fn number_params_emit_bar_values_and_pan_uses_marker_format() {
    ...
    assert!(pan.lines.windows(2).any(|pair| pair[0] == "  Pan Pos:" && pair[1] == "  C"));
    let pan_value_row = pan.selected_row.unwrap() + 1;
    assert!(matches!(pan.bar_values.get(pan_value_row), Some(Some(NativeMenuBarValue { style: Some(style), .. })) if style == "marker"));

    let mut menu = NativeMenuModel::new(config());
    menu.state.stack = vec![5, 1];
    menu.state.cursor = 0;
    let volume = menu.snapshot();
    assert!(volume.lines.windows(2).any(|pair| pair[0] == "  Master Vol:"));
    assert!(matches!(volume.bar_values.get(volume.selected_row.unwrap() + 1), Some(Some(NativeMenuBarValue { frac_pct: 100, style: None, .. }))));
...
fn mixer_menu_uses_current_volume_and_pan_values() {
    ...
    let NativeMenuValue::Number { value, .. } = menu.current_siblings()[1].value else { panic!("volume should be number"); };
    assert_eq!(value, 70);
    menu.state.cursor = 2;
    let pan = menu.snapshot();
    assert!(pan.lines.windows(2).any(|pair| pair[0] == "  Pan Pos:" && pair[1] == "  L6"));
}
...
fn fx_slot_groups_show_selected_effect_params() {
    ...
    config.fx_buses[0].slot1_params = serde_json::json!({ "timeMs": 333, "feedback": 0.42, "mixPct": 44 });
    config.global_fx_slots[0] = "vinyl".into();
    config.global_fx_params[0] = serde_json::json!({ "cracklePct": 9, "warpDepthPct": 6, "mixPct": 80 });
    let mut menu = NativeMenuModel::new(config);
    menu.state.stack = vec![2, 1, 0, 0];
    assert!(menu.current_siblings().iter().any(|item| item.label == "Feedback" && matches!(item.value, NativeMenuValue::Number { value: 42, .. })));
    menu.state.stack = vec![2, 2, 0];
    assert!(menu.current_siblings().iter().any(|item| item.label == "Crackle %" && matches!(item.value, NativeMenuValue::Number { value: 9, .. })));
}
